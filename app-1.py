...
# Add logo (optional): st.image("logo.png", width=150)
...
scaler = MinMaxScaler()
lstm_data = scaler.fit_transform(features[['Open', 'High', 'Low', 'Close', 'Volume']])
X_lstm, y_lstm = [], []
for i in range(10, len(lstm_data)):
    X_lstm.append(lstm_data[i-10:i])
    y_lstm.append(labels.iloc[i])
X_lstm, y_lstm = np.array(X_lstm), np.array(y_lstm)

model_lstm = Sequential()
model_lstm.add(LSTM(50, return_sequences=False, input_shape=(X_lstm.shape[1], X_lstm.shape[2])))
model_lstm.add(Dense(1, activation='sigmoid'))
model_lstm.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])
model_lstm.fit(X_lstm, y_lstm, epochs=3, batch_size=16, verbose=0)

lstm_prediction = model_lstm.predict(X_lstm[-1].reshape(1, 10, 5))[0][0]
st.write(f"LSTM Bullish Probability: {lstm_prediction:.2%}")
